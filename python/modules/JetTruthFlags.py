...
class JetTruthFlags(Module):
    ...
    def __init__(
        ...
        flags={
            ...
            'isLLP_Merged':['isLLP_QMU', 'isLLP_QQMU', 'isLLP_QQ'],
            'isLLP_Q':['isLLP_QQ', 'isLLP_Q'],
            'isLLP_MU':['isLLP_MU']
        },
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        if not self.globalOptions['isData']:
            for latentVariable in self.latentVariables:
                self.out.branch(self.outputName+"_"+latentVariable,"F",lenVar="n"+self.outputName)
    ...
    def analyze(self, event):
        ...
        for ijet,jet in enumerate(jets):
            ...
            for latentVariable in self.latentVariables:
                latentVariableDict[latentVariable][ijet] = getattr(jetOrigin[jet._index], latentVariable)
        ...
        if not self.globalOptions['isData']:
            for latentVariable in self.latentVariables:
                self.out.fillBranch(self.outputName+"_"+latentVariable, latentVariableDict[latentVariable])
