...
class PhiXYCorrection(Module):
    def __init__(self, era='2019', metInputDict={"met": "MET"}, isMC=1, isUL=0, sys='xycorr', metObject=False):
        self.era = era
        self.isMC = isMC
        self.isUL = isUL
        self.sys = sys
        self.metInputDict = metInputDict
        self.object = metObject
        self.xy_corr = {
            # isUL==0
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        for metInput in self.metInputDict.keys():
            self.out.branch(metInput+"_"+self.sys+"_pt", "F")
            self.out.branch(metInput+"_"+self.sys+"_phi", "F")
    ...
    def analyze(self, event):
        ...
        for metInput, metObject in self.metInputDict.iteritems():
            if self.object:
                met = Object(event, metObject)
            else:
                met = getattr(event, metObject)

            uncormet = met.pt
            uncormet_phi = met.phi
            ...
            self.out.fillBranch(metInput+"_"+self.sys+"_pt", CorrectedMET)
            self.out.fillBranch(metInput+"_"+self.sys+"_phi", CorrectedMETPhi)

            CorrectedMETObject = Object(event, metInput+"_"+self.sys)

            setattr(CorrectedMETObject, "pt", CorrectedMET)
            setattr(CorrectedMETObject, "eta", 0)
            setattr(CorrectedMETObject, "phi", CorrectedMETPhi)

            setattr(event, metInput+"_"+self.sys, CorrectedMETObject)
