...
lumis = {2016: 35.92, 2017: 41.53, 2018: 59.74}


class DataFlag(Module):
    def __init__(self,yields, globalOptions={"isData":False}):
        self.globalOptions=globalOptions
        self.lumi = lumis[self.globalOptions["year"]]
        with open(os.path.expandvars(yields)) as f:
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.xsecweight = 0.
        proc_name = inputFile.GetName().split("/")[14]
        for processId,name in enumerate(sorted(self.xsecs.keys())):
            if inputFile.GetName().find(name)>=0:
                self.xsecweight = self.xsecs[name]/self.genweights[proc_name]
    ...
    def analyze(self, event):
        ...
        if not self.globalOptions["isData"]:
            self.out.fillBranch("xsecweight",self.xsecweight*event.Generator_weight*self.lumi*1000.)
        else:
